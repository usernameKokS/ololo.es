...
class TariffController extends Controller
{
    ...
    public function tariffDetail($postId, $plan, Request $request)
    {
        ...
        if ($request->has('action') && in_array($request->action, ['publish', 'edit', 'delete'])) {
            Post::find($postId)->update(['action' => $request->action, 'pay_status' => 'wait']);
        }
                ...
                if ($payment->isThreedsV2()) {
                    ...
                    <form id="ThreeDForm" method="POST" action="<?= $url ?>">
                        ...
                        <input type="hidden" name="TermUrl"
                               value="https://<?= $request->server('HTTP_HOST') ?>/t7x4jwvkt6nhkv8fzyrm97fqrhdn5nx2m5m2/cardinity-back""
                        />
